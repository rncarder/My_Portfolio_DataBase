...
3/5/26 - 3/14/26
Moved all the logic off the forms and seperated the bussiness logic, ui logic, and the connections made to the database. i Put all business logic in the .core(dll) I created another script inside the entryapp(ui) and put all ui logic inside of that. now the forms only handle data binding to the controls. the ui helper essiential just validates that the controls are not empty before adding the newly created dtos to the buffers. the .core ot the bussiness logic formats alll the strings, checks for duplicates and creates the dto. I put the all connection logic on a script of its own which handles getting data from the database to put in comboboxes or drop downs ie(since every episode belongs in a season it gathers all seasons from the the database and put them in a dictionary with the season name as the key and a readDto as the key.) also the connection script is responsible for adding new data entered into the database. it uses a unit of work pattern to do so.


3/20/26 - 3/27/26
I created a new dll for file handling. I wanted a way to be able to create logs and to save buffers as csvs so just in case of a crash the users work can be saved i created the log to tell the programmers what exceptions took place a possibly give them where and why the system crash if it does. the log has its own class that consists of a log message which tells where the error or exception was made a description that essientailly gives the exception message and datetime and which user was using the system when the crash happened. the user is kind of pointless at the moment as i dont yet have a userrs table.

4/5/26
i moved the dto classes the interfaces and the server connection to the the schema figured data would be better protected that way as i plan on setting the models to internal instead of public so the server connection script can still have access to the models but nothing else can. i moved the dtos beause of the references are setup as the ui ref's the dll the dll refs the schema so prevent circular dependencies I had to move the dtos and the interfaces to the schema so the server connection script can still access them as can everything else. i also scrapped the file handling dll as it was going to cause alot of problems with dependencies and really i put LogWriter dto in a parameter in the file handler so it just wasnt going to work as a seperate dll as i thought so i put it all in one script in the schema for now.
